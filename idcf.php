<?php
/**
Plugin Name: IDCF Cloud ObjectStorage
Plugin URI: http://wordpress.org/extend/plugins/wp-idcfcloud-objectstorage/
Description: Plugin for IDCF Cloud ObjectStorage.
Version: 1.0
Author: mats116
Author URI: https://github.com/mats116
License: GPLv2
 */
...
class WordpressReadOnly extends WordpressReadOnlyGeneric {
	...
	* * * * * * * * * * * * * * * * * * * * * * */
	...
	function admin_menu() {
		add_options_page('IDCF Cloud ObjectStorage Plugin Settings', 'IDCF Cloud ObjectStorage', 'manage_options', 'wpro', array($this, 'admin_form'));
	}
	function admin_form() {
		...
		?>
			...
			<div class="wrap">
				...
				<h2>IDCF Cloud ObjectStorage</h2>
...
<!--								<input name="wpro-service" id="wpro-service-klandestino" type="radio" value="klandestino" <?php if ($wproService == 'klandestino') echo ('checked="checked"'); ?>/> <label for="wpro-service-klandestino">Klandestino CDN</label><br />-->
					...
					<div class="wpro-service-div" id="wpro-service-s3-div" <?php if ($wproService == 'klandestino') echo ('style="display:none"'); ?> >
						<h3><?php echo __('ObjectStorage Settings'); ?></h3>
						<table class="form-table">
							<tr>
								<th><label for="wpro-aws-key">API Key</label></th> 
								<td><input name="wpro-aws-key" id="wpro-aws-key" type="text" value="<?php echo get_option('wpro-aws-key'); ?>" class="regular-text code" /></td>
							</tr>
							<tr>
								<th><label for="wpro-aws-secret">Secret Key</label></th> 
							...
							<tr>
								<th><label for="wpro-aws-bucket">Bucket</label></th> 
								<td>
							...
							<tr>
								<th><label for="wpro-aws-endpoint">Region</label></th> 
								<td>
									<select name="wpro-aws-endpoint" id="wpro-aws-endpoint">
										<?php
											$aws_regions = array(
												'ds.jp-east.idcfcloud.com' => 'EAST (jp-east)',
												'ds.jp-west.idcfcloud.com' => 'WEST (jp-west)'
											);
...
//		$this->debug(print_r($data, true));
		...
		default:
			if (get_option('wpro-aws-virthost')) {
				...
				$data['baseurl'] = 'http://' . trim(str_replace('//', '/', get_option('wpro-aws-bucket') . '.' . get_option('wpro-aws-endpoint') . '/' . trim(get_option('wpro-folder'))), '/');
			}
		}
